...
class HUDAudioEngine {
  ...
  playSpeakingStart() {
    ...
    osc.frequency.setValueAtTime(1567.98, now + 0.06);
    gain.gain.setValueAtTime(0.1, now);
...
const hudAudio = new HUDAudioEngine();
...
// Add tactile click to settings toggles
['sfx-toggle', 'tts-toggle', 'continuous-toggle', 'model-select', 'voice-select'].forEach(id => {
  const el = document.getElementById(id);
  if (el) {
    el.addEventListener('change', () => hudAudio.playClick());
  }
});
